//
//  SnapshotRetentionPolicy.swift
//  https://github.com/mochidev/CodableDatastore
//
//  Created by Dimitri Bouniol on 2024-09-02.
...
public struct SnapshotRetentionPolicy: Sendable {
    ...
    typealias PrunePredicate = @Sendable (_ now: Date, _ creationDate: Date, _ distance: Int) -> Bool
    ...
    public static let none = SnapshotRetentionPolicy(isNone: true) { _, _, _ in true }
    ...
    public static let indefinite = SnapshotRetentionPolicy(isIndefinite: true) { _, _, _ in false }
    ...
    public static func transactionCount(_ count: Int) -> Self {
        ...
        return SnapshotRetentionPolicy { _, _, distance in distance > count}
    }
    ...
    public static func duration(_ timeInterval: TimeInterval) -> Self {
        SnapshotRetentionPolicy { now, creationDate, _ in creationDate < now.addingTimeInterval(-timeInterval) }
    }
    ...
    public static func both(_ lhs: SnapshotRetentionPolicy, and rhs: SnapshotRetentionPolicy) -> Self {
        ...
        return SnapshotRetentionPolicy { lhs.shouldIterationBePruned(now: $0, creationDate: $1, distance: $2) && rhs.shouldIterationBePruned(now: $0, creationDate: $1, distance: $2)}
    }
    ...
    ///
    /// - Parameter now: The current time to check against.
    /// - Parameter creationDate: The creation date to check.
    /// - Parameter distance: How far the iteration is from the current root. The current root is `0` away from itself, while the next oldest iteration has a distance of `1`.
    /// - Returns: `true` if the iteration, all its ancestors, and all it's other decedents should be pruned, `false` if the next iteration should be checked.
    func shouldIterationBePruned(now: Date = Date(), creationDate: Date, distance: Int) -> Bool {
        shouldPrune(now, creationDate, distance)
    }
}
